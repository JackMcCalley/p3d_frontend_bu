...
class AltSourceUploadDCM extends Component {
    constructor(props){
        super(props)
        this.state={
            file: null,
    ...
    componentWillMount(){
        ...
        fetch(`${apiUrl}/user_by_email/${this.props.location.state.email}`).then((rawResponse)=>{
          ...
          this.setState({user: userData, licenseAgreementAccepted: this.props.location.state.licenseAgreementAccepted,
             ...
             dentist: this.props.location.state.email})
        })
    }
    ...
    render() {
        ...
        if(this.state.fileUploadSuccess==true && this.state.currentUrl=="/alternate-source/upload/dcm"){
            uploadSuccess =
                <div>
                  ...
                  <Row>
                    ...
                    <Column style={styles.formColumn2}>
                      <span style={{marginTop: '5px'}}>
                        Your .dcm files MUST be compressed and in .zip format before upload.
                      </span>
                      <form>
                          <input
...
export default AltSourceUploadDCM
