...
class RXFormTreatmentPlanning extends Component {

    constructor(){
        super()
        this.state={
          serviceType: "Digital Treatment Planning",
          doctor: '',
          email: '',
          case: 0,
          address: '',
      ...
      componentWillMount(){
          if(typeof this.props.location.state == 'undefined' || !this.props.location.state.licenseAgreementAccepted){
              this.props.history.replace('/digital-treatment-planning')
          }
      ...
      validate(serviceType, doctor, email, address, phone, patient, checked, typeChecked, implantLocations, implantSystem, guidedSurgerySystem,
        drillLengths, keySizeOuterDiameter, stlChecked){
          return {
            ...
            typeChecked: typeChecked == false,
            implantLocations: implantLocations.length == 0,
            implantSystem: implantSystem.length == 0,
            guidedSurgerySystem: guidedSurgerySystem.length == 0,
            drillLengths: drillLengths.length == 0,
            keySizeOuterDiameter: keySizeOuterDiameter.length == 0,
            stlChecked: stlChecked == false
          }
      }
      ...
      handleFormSubmit(event) {
          ...
          fetch(`${apiUrl}/rxforms`,
            ...
            if (res.status  !== 422) {
                let redirect;
                if (this.state.hasStl){
                    redirect = '/digital-treatment-planning/upload/stl'
                } else{
                    redirect = '/digital-treatment-planning/upload/dicom'
                }
                this.props.history.push({
                    pathname: redirect,
                    state: {from: this.props.location.pathname, licenseAgreementAccepted: this.props.location.state.licenseAgreementAccepted, case: this.state.case, hasStl: this.state.hasStl, service: this.state.serviceType}
                })
            }
          });
      }
    ...
    render(){
      ...
      const errors = this.validate(this.state.serviceType, this.state.doctor, this.state.email, this.state.address, this.state.phone, this.state.patient,
        this.state.checked, this.state.typeChecked, this.state.implantLocations, this.state.implantSystem, this.state.guidedSurgerySystem,
        this.state.drillLengths, this.state.keySizeOuterDiameter, this.state.stlChecked)
      ...
      if(!this.state.typeChecked){
        ...
        <div>
          <Row>
          <FormGroup style={{ marginRight: '2%'}} controlId="formHorizontal">
            <Button disabled onClick={this.handleFormSubmit.bind(this)} bsStyle="primary">
                Next >
            </Button>
          </FormGroup>
                                    ...
                                    <FormGroup style={styles.formgroup} controlId="formHorizontal">
                                        ...
                                        <Col sm={1}>
                                            <input
                                                ...
                                                type="radio"
                                                onClick={() => this.setState({ maxillary: true, mandibular: false, typeChecked: true})}
                                            />
                                        </Col>
                                        ...
                                        <Col sm={1}>
                                            <input
                                                ...
                                                type="radio"
                                                onClick={() => this.setState({ maxillary: false, mandibular: true, typeChecked: true})}
                                            />
                                        </Col>
                                    </FormGroup>
                                        ...
                                        <Col sm={1}>
                                            <input
                                                ...
                                                onClick={() => this.setState({ boneReductionGuide: false, boneLevelGuide: false,
                                                    tissueLevelGuide: false, toothSupportedGuide: true, checked: true})}
                                            />
                                        </Col>
                                    </FormGroup>
                                ...
                                <FormGroup style={styles.formgroup} controlId="formHorizontal">
                                    ...
                                    <Col sm={1}>
                                        <input
                                            ...
                                            type="radio"
                                            onClick={() => this.setState({ boneReductionGuide: false, boneLevelGuide: false,
                                                tissueLevelGuide: true, toothSupportedGuide: false, checked: true})}
                                        />
                                    </Col>
                                </FormGroup>
                            </Col>
                            <Col>
                                <Row>
                                    <FormGroup style={styles.formgroup} controlId="formHorizontal">
                                      ...
                                      <Row style={{marginTop: '3%'}}>
                                        <Col componentClass={ControlLabel} sm={8} style={{color: 'black', fontSize: '14px'}}>
                                      ...
                                      <Row style={{marginTop: '10%', color: 'black'}}>
                                          <input checked={this.state.hasStl == true} type="radio" onClick={() => this.setState({ hasStl: true, stlChecked: true })}/>
                                          &nbsp;I will be uploading my own .stl file
                                      </Row>
                                      <Row style={{marginTop: '5%', color: 'black'}}>
                                          <input checked={this.state.hasStl == false} type="radio" onClick={() => this.setState({ hasStl: false, stlChecked: true })}/>
                                          &nbsp;I will be sending in stone models to P3D
                                      </Row>
                                      <Row style={{marginTop: '5%', color: 'black'}}>
                                        <b>5600 South Old Spanish Trail, Tucson, AZ, 85747</b>
                                      </Row>
                                    </FormGroup>
                                </Row>
                            </Col>
...
export default RXFormTreatmentPlanning
